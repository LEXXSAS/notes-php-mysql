...
if (isset($_POST["currentpage"]) && $_POST["prioritynext"]) {
  $rowperpage = 4;
  $currentpage = $_POST["currentpage"];
  $prioritynext = $_POST["prioritynext"];
  $searchvaluenext = $_POST["searchvaluenext"];
  $starttwo = ++$currentpage;
  $page = $starttwo * $rowperpage;
  if ($prioritynext === 'low' || $prioritynext === 'middle' || $prioritynext === 'high') {
    $countsql = $pdo->prepare("SELECT COUNT(*) FROM notes WHERE priority=?");
    $countsql->execute([$prioritynext]);
    $countresult = $countsql->fetchColumn();
    $pages = ceil($countresult / $rowperpage);
    $sql = ("SELECT * FROM notes WHERE priority=? LIMIT $page, $rowperpage");
    $query = $pdo->prepare($sql);
    $query->execute([$prioritynext]);
    $starnewpage = $starttwo;
    ++$starnewpage;
  }
  else if ($searchvaluenext !== null || $searchvaluenext !== '') {
    $countsql = $pdo->prepare("SELECT COUNT(*) FROM notes WHERE title LIKE '$searchvaluenext%'");
    $countsql->execute();
    $countresult = $countsql->fetchColumn();
    $pages = ceil($countresult / $rowperpage);
    $sql = "SELECT * FROM notes WHERE title LIKE '$searchvaluenext%' LIMIT $page, $rowperpage";
    $query = $pdo->prepare($sql);
    $query->execute();
    $starnewpage = $starttwo;
    ++$starnewpage;
...
if (isset($_POST["nextpage"]) && $_POST['priorityprev']) {
  $rowperpage = 4;
  $currentpage = $_POST["nextpage"];
  $priorityprev = $_POST['priorityprev'];
  $searchvalueprevious = $_POST["searchvalueprevious"];
  $starttwo = --$currentpage;
  $page = $starttwo * $rowperpage;
  if ($priorityprev === 'low' || $priorityprev === 'middle' || $priorityprev === 'high') {
    $countsql = $pdo->prepare("SELECT COUNT(*) FROM notes WHERE priority=?");
    $countsql->execute([$priorityprev]);
    $countresult = $countsql->fetchColumn();
    $pages = ceil($countresult / $rowperpage);
    $sql = ("SELECT * FROM notes WHERE priority=? LIMIT $page, $rowperpage");
    $query = $pdo->prepare($sql);
    $query->execute([$priorityprev]);
    $starnewpage = $starttwo;
    ++$starnewpage;
  }
  else if ($searchvalueprevious !== null || $searchvalueprevious == '') {
    $countsql = $pdo->prepare("SELECT COUNT(*) FROM notes WHERE title LIKE '$searchvalueprevious%'");
    $countsql->execute();
    $countresult = $countsql->fetchColumn();
    $pages = ceil($countresult / $rowperpage);
    $sql = "SELECT * FROM notes WHERE title LIKE '$searchvalueprevious%' LIMIT $page, $rowperpage";
    $query = $pdo->prepare($sql);
    $query->execute();
    $starnewpage = $starttwo;
    ++$starnewpage;
